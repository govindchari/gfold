...
T step(T (*f)(T const &state, const Vector3d &force, const Vector3d &moment, const double &thrust),
       const T &x, const double &h, const Vector3d &F, const Vector3d &M, const double &Th) {
  static constexpr double half = 0.5;
  static constexpr double two = 2.0;
  static constexpr double sixth = 1.0 / 6.0;
  auto k1 = f(x, F, M, Th);
  auto k2 = f(x + half * h * k1, F, M, Th);
  auto k3 = f(x + half * h * k2, F, M, Th);
  auto k4 = f(x + h * k3, F, M, Th);
